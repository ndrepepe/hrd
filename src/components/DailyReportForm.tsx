...
const DailyReportForm = ({ onReportSubmitted, editingReportId, setEditingReportId }: DailyReportFormProps) => {
  const [employees, setEmployees] = useState<Employee[]>([]);
  const [loadingEmployees, setLoadingEmployees] = useState(true);
  const [loadingReportData, setLoadingReportData] = useState(false); // State for loading report data for edit
  ...
  // Effect to fetch employees for the select dropdown
  useEffect(() => {
    fetchEmployees();
  }, []);
  ...
  useEffect(() => {
    if (editingReportId) {
      const fetchReport = async () => {
        setLoadingReportData(true); // Start loading report data
        console.log("Fetching report data for edit:", editingReportId);
        const { data, error } = await supabase
          .from("daily_reports")
        ...
        setLoadingReportData(false); // End loading report data
        ...
        if (error) {
          ...
          form.reset({
           ...
           console.warn("Report data not found for ID:", editingReportId);
      ...
      console.log("Resetting report form to default values.");
      form.reset({
        report_date: undefined,
        employee_id: "",
        activity: "",
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    console.log("Form submitted with values:", values, "Editing ID:", editingReportId);
    ...
    if (editingReportId) {
      ...
      result = await supabase
        ...
        .update(reportData)
        .eq("id", editingReportId)
        .select(); // Use select() to get the updated data
    } else {
      // Add new report
      result = await supabase
        .from("daily_reports")
    ...
    if (error) {
      ...
      console.log(`Daily report data ${editingReportId ? 'updated' : 'inserted'} successfully:`, data);
  ...
  return (
    <div className="w-full max-w-lg mx-auto">
      <h3 className="text-xl font-semibold mb-4">{editingReportId ? "Edit Data Laporan Harian" : "Input Laporan Harian"}</h3>
      {loadingReportData ? (
